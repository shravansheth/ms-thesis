...
```text
ClangIR build:
  clang with -fclangir support
  cir-opt with --cir-to-mlir support
...
RPi link flags used by the scripts:
  zig cc --target=aarch64-linux-musl -static -O2
```

The scripts use `LLVM_BUILD` to locate the MLIR/LLVM tools. The ClangIR tools
are only needed when regenerating the C-to-MLIR artifacts; the packaged final
benchmark runs start from the generated `.O2.ll` files.

...
The pass system has two components:
...
Both passes were implemented from stubs. The final state of each file is described below.
...
Three forms are recognized (dim 0 only, which covers all 5 cases):
...
- `func.call` that is noinline → descend into the callee function body. For each argument position where `arg == v`, find the `BlockArgument` in the callee and tag its load/store users directly (no further recursion).
...
### Verified output (all 5 kernels)

All 5 kernels produce correct tagging when run through `alias-meta-opt --mark-alias-groups`:
...
Note: `alias-meta-opt` only has our two passes registered (not the full suite of standard MLIR passes). It is used exclusively for the two-step pipeline:
1. `alias-meta-opt` runs our passes

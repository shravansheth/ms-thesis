...
The implementation is **two consecutive MLIR passes** inserted between
`-fold-memref-alias-ops` and `-expand-strided-metadata`:
...
For the **noinline callee** case (`subview_noalias`, `tiling_noinline`): instead of
marking loads/stores (which are inside the callee and don't have subview context), record
that a specific `func.call` passes disjoint-pair subviews as arguments at positions `(i, j)`.
Store this as an attribute on the `func.call` op itself.

**Output**: `memref.load` / `memref.store` ops tagged with `#alias_group<id>`, and
`func.call` ops tagged with disjoint argument-pair information.
...
At the `func.call` op tagged with disjoint argument pair `(i, j)`:
1. Look up the called `func.func` by symbol
2. Add `llvm.noalias` parameter attributes to the pointer parameters at positions `i` and `j`
   of the callee function signature (using `LLVM::LLVMFuncOp::setArgAttr`)
3. Remove the marker attr from the `func.call`

`noalias` on function parameters is the correct LLVM mechanism for this case: it asserts
that within the scope of this call, the memory regions pointed to by those arguments do
not overlap. LLVM's LICM and AA use this directly inside the callee without any changes
to the callee's body. If a callee has multiple call sites with different aliasing
contexts, `noalias` is only added where the structural proof holds — the pass handles
each `func.call` independently.
...
| | Pass 1 `MarkAliasGroups` | Pass 2 `LowerWithAliasMeta` |
|---|---|---|
| Base class | `OperationPass<ModuleOp>` | `OperationPass<ModuleOp>` using `DialectConversion` |
| Pattern matching | Walk `memref.subview`, check `addi` def chain | `ConversionPattern` on marked `memref.load`/`store` |
| Key MLIR APIs | `op.getDefiningOp()`, `op.getUses()`, `IntegerAttr` | `MemRefDescriptor`, `LLVMTypeConverter`, `LLVM::AliasScopeAttr` |
| Noinline case | Tag `func.call` with arg pair info | `setArgAttr(llvm.noalias)` on callee params |
| Does NOT modify | `finalize-memref-to-llvm` or any upstream pass | Any unmarked ops |
...
- Clone or specialize callees — `noalias` parameter attributes are per-call-site
